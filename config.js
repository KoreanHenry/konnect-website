...
//   제목, 날짜, 시간, 카운트다운 모두 자동 반영.
//
//   형식: 'YYYY-MM-DDTHH:MM:SS+09:00'
//   예시: 2026년 6월 9일 오후 1시 30분
...
function applyConfig() {
  ...
  // ② T 번역 객체 (index.html & join.html 공통)
  if (typeof T !== 'undefined') {
    ['ko','en'].forEach(l => {
      if (!T[l]) return;
      T[l].evTitle  = M[l].title;
      T[l].evDate   = '📍 '+M[l].location+' · '+M[l].date+' · '+M[l].time;
      ...
      // join.html 전용
      if (T[l].ev2 !== undefined) {
        T[l].ev2    = M[l].date;
        T[l].ev3    = M[l].time;
        T[l].ev4    = M[l].location;
        T[l].ctaKr  = M[l].ctaKr;
        T[l].ctaIntl= M[l].ctaIntl;
      }
    });
  ...
  document.querySelectorAll('[data-spots]').forEach(el=>{
    ...
    if(CONFIG.SPOTS_LEFT) el.textContent=CONFIG.SPOTS_LEFT+(lang==='ko'?'자리 남음':' spots remaining');
  });
